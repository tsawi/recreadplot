...
for iev, event in enumerate(eventlist) :
    ...
    evtime = np.append(evtime,eventlist[iev].origins[0].time.strftime("%d/%m/%y %H:%M"))
...
def save_button_callback():
    eventdat = pd.Series(data={'ID':input_ID.value,'lat':input_lat.value, 'lon':input_lon.value, 'time':input_time.value, 'mag':input_mag.value})
...
# Display plots
bokeh_doc = curdoc()
bokeh_doc.add_root(column(Div(text='<h1> Select event <h1>'),(row(column([input_lat,input_lon,search_rad,input_time,search_time_range,input_mag,mag_type,input_webservice,save_button,quit_button]),p))))
bokeh_doc.title = "Select Event"