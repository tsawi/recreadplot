function [traces] = read_sac_resp(respn,traces)
...
%
% INPUT
% respn: input resp array
% traces: input traces structure
% 
% OUTPUT
% zz: zeros
% pp: poles
% constant: (final sensitivity) * (A_0)
% units: 'M' or 'M/S'
%
% Josh Russell 20/2

    % Units for output poles and zeros
    unit = 'M'; % M (displacement) | M/S (velocity)
    
    % Split on newlines
    resp = splitlines(respn);
...
%     end
    ...
    % Add fields to traces sacpz
    for itr = 1:length(traces)
        traces(itr).sacpz.units = unit;
        traces(itr).sacpz.constant = const;
        traces(itr).sacpz.poles = pp;
        traces(itr).sacpz.zeros = zz;
    end
